...
def movie_review_crawler(movie_code):
    ...
    # "2480" -> 2480 (O)
    # "2,480" -> 문자 포함되어 있으면 변환X

    # 1. 숫자만 추출 : 정규식
    numbers = re.sub(r'[^0-9]', '', all_count)
    pages = math.ceil(int(numbers)/10)
    print(f'The total number of pages to cellect is {pages}')
    ...
    for page in range(1, pages+1):
        ...
        for i, one in enumerate(review_list):  # review 1건씩 수집
            ...
            # review = one.select('span[id *= "_filtered_ment"]')[0].get_text().strip()
            ...
            # Review 데이터 생성
            # -> 규격(포맷) -> JSON
            # JSON -> 데이터 주고 받을 때 많이 사용하는 타입
            # MongoDB -> BSON(Binary JSON) = JSON
            # Python의 Dictionary = JSON
            #
            # 결론 : Python Dictionary = JSON = BSON
            # JSON 포맷
